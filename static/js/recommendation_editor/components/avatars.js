...
function calculateDynamicAvatarSize(participants, plotHeight, isModalOpen = false) {
    const maxStackSize = calculateMaxStackSize(participants);
    const availableHeight = plotHeight - CONFIG.PLOT.BOTTOM_MARGIN;
...
export function updateSingleParticipant(participantData, loadModalCallback) {
    ...
    if (!avatar) {
        avatar = createAvatarElement(participantData, loadModalCallback, avatarSize);
        container.appendChild(avatar);
    } else {
        updateExistingAvatar(avatar, participantData);
    }

    // Animate to position with calculated size
    animateAvatarToPosition(avatar, participantData, plot, avatarSize);
...
export function updateAvatars(results, loadModalCallback) {
    ...
    results.forEach((participant, index) => {
        // Create avatar with dynamic size
...
function repositionExistingAvatars() {
    ...
    const isModalOpen = document.body.classList.contains('participant-modal-open') ||
                        document.body.classList.contains('meta-panel-open');
    const participants = Array.from(AppState.currentParticipants.values());
    const avatarSize = calculateDynamicAvatarSize(participants, plotHeight, isModalOpen);
